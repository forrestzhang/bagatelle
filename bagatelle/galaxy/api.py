...
modify from galay script
get_histories => get_history_contents =>  get_history_files=> filter_filename=> for loop downloadfile
"""
...
def make_url(api_key, url, args=None):
    ...
    #     print(url + argsep + '&'.join(['='.join(t) for t in args]))
...
def get_histories(api_key, ip_address):
    url = 'http://' + ip_address + '/api/histories/'
    return display(api_key, url, return_formatted=False)


def get_history_contents(api_key, ip_address, history_id):
    url = 'http://' + ip_address + '/api/histories/' + history_id + '/contents'
    print(url)
    return display(api_key, url, return_formatted=False)


def get_history_files(history_contents):
    fileurl = dict()
    for i in history_contents:
        fileurl[i['url']] = i['name']
    return fileurl


def filter_filename(fileurl, patten):
    res = dict()

    for i in fileurl:

        #         print(patten, fileurl[i])

        m = re.search(patten, fileurl[i])
...
def downloadfile(api_key, ip_address, api_url, filename):
    url = make_url(api_key, 'http://' + ip_address + api_url + '/display')
    try:
        print(url, filename)
